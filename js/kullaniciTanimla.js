    ...
    function YetkiListesi() {
        $.ajax({
            ...
            success: function (gelen) {
                ...
                for (var i = 0; i < gelen.length; i++) {
                    var yetki = gelen[i];
                    // Her bir yetki için option elementi oluştur
                    _html += "<option value='" + yetki.YetkiId + "' data-kampus='" + yetki.KampusId + "'>" + yetki.YetkiAd + " - " + yetki.KampusAd + "</option>"
                }
                $("#editYetki").html(_html);
                $("#yetkiId").html(_html);
                 // Optionları yetkiId elementine ekle
            },
            error: function(xhr, status, error) {
     ...
     $("#kullaniciKaydet").click(function () {
        ...
        var _YetkiId = $("#yetkiId").val(); // Seçilen yetkiId'yi al
